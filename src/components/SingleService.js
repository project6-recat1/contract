...
const SingleService = () => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post('http://localhost/brief6/contractAPI/addUsercontracrt.php', inputs).then(function (response) {
      console.log(response.data);
    });
  };
            ...
            {tiers.map((tier) => (
              ...
              <Grid item key={tier.title} xs={12} sm={tier.title === 'Enterprise' ? 12 : 6} md={4}>
                <Card>
                  ...
                  <CardActions>
                    <Button fullWidth variant={tier.buttonVariant} onClick={() => handleOpen(tier.expiryDate, tier.price)}>
                      {tier.buttonText}
                    </Button>
                  </CardActions>
                </Card>
